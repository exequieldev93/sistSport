...
<html lang="en">
  ...
  <body>
    ...
    <header class="clearfix">
        <!--Logo-->  
        <div id="logo">
            <img src="C:\Users\Usuario\Desktop\Trabajo Final\5_Sistema\sistSport\public\imagenes\logos\{{$empresa->imagen}}" alt="Logo">
        </div>
        <!--Empresa-->
        <div id="company">
            <h2 class="name">Empresa:{{$empresa->nombre}}</h2>
    ...
    <main>
      ...
      <table border="0" cellspacing="0" cellpadding="0" style="margin-top: 30px">
        ...
        <tbody>
            @foreach ($prendas as $i => $pre)
            <tr>
                <td >{{$i+1}}</td>
                <td class="desc">{{$pre->categoria}}</td>
                <td class="desc">{{$pre->nombre}}</td>
                <td class="desc">{{$pre->color}}</td>
                <td class="desc">{{$pre->talle}}</td>
